...
class MetaKnowledgeService:
    ...
    async def _build_core(self, meta_config: MetaConfig):
        """元知识库构建核心逻辑（接受 MetaConfig 对象）"""
        # 0. 清除旧的运行时数据，避免主键冲突
        session = self.meta_mysql_repository.session
        if session.in_transaction():
            await self.meta_mysql_repository.clear_runtime_data()
            await session.commit()
        else:
            async with session.begin():
                await self.meta_mysql_repository.clear_runtime_data()
        logger.info("已清除旧的元知识库运行时数据")
    ...
    async def build_from_config(self, meta_config: MetaConfig):
        """从 MetaConfig 对象构建元知识库（供接口调用）"""
        await self._build_core(meta_config)

    async def build(self, config_path: Path):
        """从 yaml 配置文件构建元知识库（供 CLI 调用）"""
