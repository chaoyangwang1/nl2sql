...
async def add_extra_context(state: DataAgentState, runtime: Runtime[DataAgentContext]):
    ...
    try:
        ...
        writer({"type": "progress", "step": "添加额外上下文信息", "status": "success", "elapsed": elapsed})
        logger.info(f"额外上下文信息：数据库信息-{db_info} 日期信息-{date_info}, 耗时: {elapsed}s")
        return {
            "date_info": date_info,
            "db_info": db_info,
        }
        ...
        elapsed = round(time.perf_counter() - start_time, 3)
        writer({"type": "progress", "step": "添加额外上下文信息", "status": "error", "elapsed": elapsed})
        logger.error(f"添加上下文失败:{str(e)}, 耗时: {elapsed}s")
        raise
from datetime import datetime
