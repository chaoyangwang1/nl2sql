...
async def merge_retrieved_info(state: DataAgentState, runtime: Runtime[DataAgentContext]):
    ...
    try:
        ...
        # 发送召回详情事件（供评估系统捕获）
        recall_tables = [table_info['name'] for table_info in table_infos]
        recall_columns = [
            f"{table_info['name']}.{col['name']}"
            for table_info in table_infos
            for col in table_info.get('columns', [])
        ]
        recall_metrics = [metric_info['name'] for metric_info in metric_infos]
        writer({"type": "recall_detail", "tables": recall_tables, "columns": recall_columns, "metrics": recall_metrics})
